...
- `互換モード` もしくは `compat`: TODO。
- `既知ヘルパー` もしくは `knownHelpers`: TODO。
- `既知ヘルパー限定` もしくは `knownHelpersOnly`: TODO。
- `HTMLエスケープ無効化` もしくは `noEscape`: 真を指定すると HTML のエスケープを行わなくなります。デフォルトは偽です。
- `厳格モード` もしくは `strict`: TODO。
- `オブジェクト存在確認無効化` もしくは `assumeObjects`: TODO。
- `自動インデント無効化` もしくは `preventIndent`: TODO。
- `スタンドアロンタグ空白除去無効化` もしくは `ignoreStandalone`: TODO。
- `パーシャル暗黙コンテキスト無効化` もしくは `explicitPartialContext`: TODO。
